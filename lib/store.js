   ...
   */
	async set(sid, session, callback) {
		...
		try {
			...
			const thatSession = this.sessions[sid];
			// Ensure that the Cookie in the `session` is safely serialized
			const sess = {};
			Object.keys(thatSession).forEach(key => {
				if (key === 'cookie' && typeof thatSession[key].toJSON === 'function') {
					sess[key] = thatSession[key].toJSON();
				} else {
					sess[key] = thatSession[key];
				}
			});
			await this.pouch.put(sess);
			callback();
		} catch (err) {
			callback(err);
		}
